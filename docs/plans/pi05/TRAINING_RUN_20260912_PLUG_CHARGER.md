...
当前状态：1500步训练、最终保存和冻结/参数/optimizer重载全部完成，15:51:00 CST正常退出，
总耗时44分23秒。模型短名 `20260912_01:1500`；训练回访已暂停，尚未执行本任务真机评估。
...
## 最终结果与推理（2026-09-12 15:51 CST完成）

- 15:49:16完成第1500次更新；15:51:00完成保存/重载并正常退出。
  `result.step=1500`，四项冻结/可训练参数/参数重载/optimizer重载检查全部true；
  MainPID=0、ExecMainStatus=0、Result=success，`active/exited`是该服务的正常结束状态。
- 首50步平均loss=0.14708，最后50步=0.02355，下降83.99%；后期趋于平稳。
- 下表是固定15个训练观测、剔除245个padding后的505个有效目标，不是真机成功率。

| checkpoint步数 | 关节MAE(rad) | 关节MAE(°) | 夹爪状态准确率 |
| --- | ---: | ---: | ---: |
| 500 | 0.025996 | 1.49 | 100% |
| 1000 | 0.019879 | 1.14 | 100% |
| 1500 | 0.017196 | 0.99 | 100% |

- 1276次资源采样无错误；显存采样峰值17.03GiB、最少余30.36GiB；
  系统最少available83.63GiB，Swap未使用。热身后每步中位数1.62秒，约4.93样本/秒。
- 最终曲线：`outputs/pi05_plug_charger_loss_20260912_final_1500.png`及同名统计JSON。
  小型训练日志、500/1000/1500评估、最终曲线和模型元信息同步到[实验资料归档](../../experiments/README.md)。
  数据与权重本体不上传；本轮未修改模型、推理逻辑或运行机器人。
- `0-5`回访已按OpenAI Docs的[计划任务流程](https://learn.chatgpt.com/docs/automations)暂停。

PolyScope保持Remote Control；与之前一样两条命令。两条入口已做不连硬件、不加载模型的dry-run，
短名唯一对应本次插座1500步模型和数据；默认K=20、去噪10步、joint servoJ 500Hz。

```bash
ur5e-pi05-infer-init 20260912_01:1500
ur5e-pi05-infer 20260912_01:1500 --execute
```

已报告节点：数据整理、启动、1000步评估（包含500步对照）、1256步进度、1500步完成与推理命令。
训练监控结束；尚无本任务真机成功率结论。
