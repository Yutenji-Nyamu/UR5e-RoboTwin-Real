...
2026-09-12当前任务为 `plug_charger` 插插座，5条成功、1216个裁剪后样本；
沿用配方并改为1500步，重复夹爪命令的二值标签边界见 [插插座训练记录](TRAINING_RUN_20260912_PLUG_CHARGER.md)。

2026-09-11当前任务改为 `stack_blocks_two_0911`，5条成功、504个裁剪后样本；
同配方1000步SFT及最终保存重载检查已完成，短名 `stack2_0911:1000`；
训练结果和两条推理命令见 [叠两个方块0911训练记录](TRAINING_RUN_20260911_STACK2.md)。
旧任务保留；同日实验请使用完整实验名区分，不能只用 `20260911_01` 后缀。
