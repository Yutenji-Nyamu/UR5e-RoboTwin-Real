...
当前状态（2026-09-12收尾复核）：1000步训练完成，冻结/参数/optimizer重载四项检查全部通过。
当前服务无训练进程；训练结果不代表新任务真机成功率。原启动时间为2026-09-11 16:00:44 CST。
...
启动时真机执行器仍按第一次 close→open 后结束。后续本地适配已经按数据契约区分一次/两次抓放，
中间释放允许再次闭爪，只在最终释放后收尾；2026-09-12归档时将此前未提交的代码与测试一并纳入Git。
这不是任务成功检测器；本次只对既有实现做离线回归，没有执行机器人。
...
当时执行的训练参数（现已完成，不要把以下记录当作要求重新训练）：
...
- `result.json`的step为1000；`frozen_unchanged`、`trainable_changed`、`checkpoint_reload`、
  `optimizer_reload`全部true。当前服务MainPID=0，ExecMainStatus=0、Result=success，无训练进程。
- 最后50步平均loss=0.02846，最初50步为0.11896，下降约76.1%。
- 固定15个训练观测、排除末尾padding：joint MAE=0.02146 rad（约1.23°），夹爪准确率92.48%。
- 原始结果与曲线已入[实验资料归档](../../experiments/README.md)：
  `evidence/repository/logs/pi05/stack3_joint5_sft_20260911_01/20260911T080045088966Z/`及
  `evidence/repository/outputs/pi05_stack3_loss_20260911_final_1000.*`。不把离线指标等同真机成功率。
