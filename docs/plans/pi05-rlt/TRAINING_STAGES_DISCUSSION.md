...
本稿区分论文方法、固定 donor 源码、已确认的组织方式和待定实现参数；不是已经运行的配置。保留已成功的 JAX π0.5，不重做示教或 VLA SFT。整体架构见 [方块接入规划](CUBE_PLAN.md)。
...
- **当前仅维护实现前讨论文档，尚未开始 RLT 开发、训练或真机 rollout。**既有 π0.5 的训练/成功记录不计入 RLT 进度。
