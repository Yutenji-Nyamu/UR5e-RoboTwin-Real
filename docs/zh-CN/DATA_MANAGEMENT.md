...
`stack_blocks_three` 同批6条中5条人工标记success，排除失败的 `20260911_154614`。
按原π0.5的克制头尾裁剪规则导出993个训练样本，保留两次抓放及最后open后至少1秒；
新数据集 `ur5e/stack_blocks_three_joint_5_v20260911`，旧cube数据/模型及raw不修改。
来源、每条裁剪秒数、颜色顺序差异与训练参数见 [叠三块记录](../plans/pi05/TRAINING_RUN_20260911_STACK3.md)。

### 采集格式分界：2026-09-08

- 更新后的采集器使用 **raw schema v3：实测关节＋TCP 双记录**，新增
  `actual_q_0..5`、`actual_qd_0..5`、主机接收时间；manifest 明确字段语义与初态。
  详见[采集契约](runbooks/collect.md)。
- 当天检查的29份升级前原始动作CSV全部没有实测关节列，最新为 `20260907_174521`。
  旧数据只有 TCP；不能因下游使用 `joint_action` 名称就当作关节数据。
- 按 **schema＋实际字段** 判断分界，不只看日期。不重标旧数据，不用 IK 冒充实测，
  不覆盖旧文件。后续已核验5条真实v3：`20260908_153254`、`153339`、`153413`、
  `153445`、`153525`（均同一日期前缀），导出414个π0.5训练transitions；
  见[审计](../plans/pi05/JOINT5_DATA_AUDIT.json)与[真机结果](../plans/pi05/PHYSICAL_RESULT_20260910.md)。
- 数据盘 `DATA_LOG.md` 同步追加这条升级日志；这里保留可随 Git 推送的分界记录。
- 双记录为以后按表示导出数据/训练 checkpoint 做准备；现有 TCP HDF5 schema v2、
  DP/ACT 转换与重播不变。
